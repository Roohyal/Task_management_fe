...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        //console.log(responseInfo.loginInfo.email, responseInfo.loginInfo.token);
        localStorage.setItem("token", responseInfo.loginInfo.token);
        localStorage.setItem("email", responseInfo.loginInfo.email);
  ...
  useEffect(() => {
    async function confirmTheToken() {
      if (confirmToken) {
        // const response = await fetch("http://localhost:8080/api/auth/login")
